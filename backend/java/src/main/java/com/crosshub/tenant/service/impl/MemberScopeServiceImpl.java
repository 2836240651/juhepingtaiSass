...
import com.crosshub.tenant.entity.UserShopScope;
...
public class MemberScopeServiceImpl implements MemberScopeService {
    ...
    public void replaceScopes(
        ...
        for (String shopId : normalizeShopIds(shopIds)) {
            ...
            String shopIdForScope = resolveShopIdForScope(tenantId, shopId);
            ...
            scope.setShopId(shopIdForScope);
    ...
    private String resolveShopPlatform(Long tenantId, String shopId, Set<String> allowedPlatforms) {
        ...
        var platformAccount = platformAccountRepository.findByIdAndTenantId(shopId, tenantId);

        if (platformAccount.isPresent()) {

            String platform = platformAccount.get().getPlatform().toLowerCase(Locale.ROOT);
    ...
    private String resolveShopIdForScope(Long tenantId, String shopId) {
        return platformAccountRepository.findByIdAndTenantId(shopId, tenantId)
                .map(this::resolvePlatformAccountScopeId)
                .orElse(shopId);
    }

    private String resolvePlatformAccountScopeId(PlatformAccount account) {
        if ("temu".equalsIgnoreCase(account.getPlatform())) {
            String external = account.getExternalShopId();
