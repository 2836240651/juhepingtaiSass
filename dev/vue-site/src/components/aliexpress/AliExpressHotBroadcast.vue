...
import {
  ...
  publishHotBroadcast,
} from '@/api/aliexpressHotBroadcast'

const props = defineProps({
  products: { type: Array, required: true },
  broadcasts: { type: Array, default: () => [] },
  storeId: { type: String, default: 'all' },
})
...
function formatSurgePercent(ratio) {
  const value = Number(ratio)
  if (!Number.isFinite(value) || value <= 0) return '—'
  if (value >= 100) return '新出单'
...
async function sendBroadcast(product) {
  ...
  try {
    ...
    localBroadcasts.value = await loadHotBroadcasts(auth, { storeId: props.storeId })
    ...
    <el-row :gutter="16">
            ...
            <el-table-column label="增幅" width="100" align="right" sortable prop="surgeRatio">
              <template #default="{ row }">
                <el-tag :type="surgeTagType(row.surgeRatio)" size="small">
                  {{ formatSurgePercent(row.surgeRatio) }}
                </el-tag>
              </template>
            </el-table-column>
      ...
      <el-col :xs="24" :lg="10">
        <el-card shadow="never">
          ...
          <el-empty v-if="!localBroadcasts.length" description="暂无通报记录，识别到爆款后可点击「通报」" :image-size="64" />
          ...
          <el-timeline v-else>
            ...
            >
              ...
              <el-text tag="p" size="small" type="info">
                通报人 {{ item.operator || '系统' }} · 当日 {{ item.dailySales }} 件 · 7 日均 {{ item.avg7DayDaily }} · 增幅 {{ formatSurgePercent(item.surgeRatio) }}
              </el-text>
        ...
        <el-card shadow="never" style="margin-top: 16px">
          ...
          <el-descriptions :column="1" size="small" border>
            ...
            <el-descriptions-item label="通报范围">全租户员工可见（Java 服务端持久化）</el-descriptions-item>
          </el-descriptions>
        </el-card>
      </el-col>
    </el-row>
